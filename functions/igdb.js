...
module.exports = (admin, functions) => {
  ...
  return {
    fetchGames: functions.runWith({ timeoutSeconds: 540, memory: '8GB' }).pubsub.schedule('0 0 * * *').onRun(fetchGames),
    fetchPlatforms: functions.pubsub.schedule('0 1 * * *').onRun(fetchPlatforms),
